...
var categories = validCategories.map((g) => g.category)
...
function findPhylogenyInInfobox(infobox) {
  ...
  // Get all phylogeny terms from validCategories
  const phylogenies = validCategories.map((g) => g.phylogeny)
...
// --- Input Data ---
export async function assessCategoryMembershipWP(binomialName, locale = 'en') {
  try {
    ...
    if (foundPhylogeny) {
      // Find the category that matches this phylogeny
      ...
      if (matchedCategory) {
        console.log(
          `Matched phylogeny "${foundPhylogeny}" to category: ${matchedCategory.category}`,
        )

        // Step 2: If it's Plantae, do secondary search for tree/grass
        if (matchedCategory.category === 'plant') {
          console.log('Detected Plantae - searching for specific plant types...')

          const plantSpecificCategories = ['tree', 'grass']
          const plantResult = getMostFrequentWord(plantSpecificCategories, fulltext, false)
          ...
          // Return the matched category directly
          return matchedCategory.category
        }
      }
    }

    // Step 3: Fallback to original category-based search in full text
    console.log('No phylogeny found in infobox, falling back to category search in full text')
    const categoryResult = getMostFrequentWord(categories, fulltext, false)

    if (categoryResult.word) {
      console.log(
        `Found category "${categoryResult.word}" in full text (${categoryResult.count} occurrences)`,
      )
      return categoryResult.word
    }

    // Step 4: Final fallback
    console.log('No categories found, defaulting to "unknown"')
    return 'unknown'
  } catch (error) {
...
export const assessGroupMembershipWP = assessCategoryMembershipWP
